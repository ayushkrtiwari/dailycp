...
{
    size_t  n = (int)P.size();
    // P[0] be our base point or top vertex for all triangles
...
template<class F1, class F2>
bool polygon_in_polygon_strict(vector<Point<F1>> &outer, const vector<Point<F2>> &inner)
{
    // ensure outer is CCW  
...
int32_t main()
{
    int n;
    cin>>n;
    vector<Point<int>> outer(n);
    for(auto &p : outer)
    cin>>p.x>>p.y;
    int m;
    cin>>m;
    vector<Point<int>> inner(m);
    for(auto &p : inner)
    cin>>p.x>>p.y;
    cout<<(polygon_in_polygon_strict(outer, inner) ? "YES" : "NO")<<"\n";
}