...
FUNC1(angle, Point<F>& point, (point.y, point.x))
...
auto signed_twicearea(const vector<Point<F>> &P) -> decltype(P[0].x * P[0].y)
{
    ...
    return A;
    // if area < 0, points are cw else ccw
}
...
static constexpr long double EPS = 1e-18;
static constexpr long double pi = acosl(-1.0);
...
int32_t main()
{
    ...
    for(auto &e : poly)
    cin>>e.x>>e.y;
    if(signed_twicearea(poly) < 0)
    reverse(poly.begin(), poly.end());
    int total = 1LL * n * (n - 1) * (n - 2) / 6LL;
    int t;
    cin>>t;
    while(t--)
    {
        Point<int> p;
        cin>>p.x>>p.y;
        if(!point_in_convex_strict(poly, p))
        cout<<0<<"\n";
        else
        {
            vector<long double> ang(n);
            // typeid(decltype(poly[0].y - p.y)).name();
            for(int i = 0; i < n; i++)
            ang[i] = atan2l(poly[i].y - p.y, poly[i].x - p.x);
            int start = n - 1;
            for(int i = 0; i < n; i++)
            ...
            {
                if(r < i + 1) r = i + 1;
                while(r + 1 < i + n and a2[r + 1] - a2[i] < pi - EPS) ++r;
                int d = r - i;
                if(d >= 2) bad += d * (d - 1) / 2;
            }
            cout<<total - bad<<"\n";
