...
// Convex Hull, Advanced Geometry

// For explanation see my solution on last part with minimal version.
...
template < class F > int read(Point < F > & point) {
    return read(point.x, point.y) / 2;
}
template < class F > int write(const Point < F > & point) {
...
template < class F > ostream & operator << (ostream & os,
    const Point < F > & point) {
    return os << point.x << ' ' << point.y;
}

template < class F > inline Point < F > makePoint(const F & x,
    const F & y) {
    return Point < F > (x, y);
}
...
template < class F > inline auto name(const arg) -> decltype(expr) {
    return expr;
}
#define FUNC2(name, arg1, arg2, expr)\
template < class F1, class F2 > \
    inline auto name(const arg1,
        const arg2) -> decltype(expr) {
        return expr;
    }
...
template < class F >
    struct Line {
        Point < F > a, ab;
        Line(): a(), ab() {}
        Line(const Point < F > & a,
            const Point < F > & b, bool twoPoints = true): a(a), ab(twoPoints ? b - a : b) {}
        Line(const F & xa,
            const F & ya,
                const F & xb,
                    const F & yb): a(xa, ya), ab(xb - xa, yb - ya) {}
...
template < class F >
    inline Line < F > makeLine(const Point < F > & a,
        const Point < F > & b, bool twoPoints = true) {
...
template < class F1, class F2 >
    bool onLine(const Point < F1 > & point,
        const Line < F2 > & line) {
...
template < class F1, class F2 >
    bool onSegment(const Point < F1 > & point,
        const Line < F2 > & seg) {
...
template < class F1, class F2 >
    distF < F1, F2 > distLine(const Point < F1 > & point,
        const Line < F2 > & line) {
...
template < class F1, class F2 >
    distF < F1, F2 > distSegment(const Point < F1 > & point,
        const Line < F2 > & seg) {
...
template < class F1, class F2, class F3 >
    void projection(const Point < F1 > & point,
        const Line < F2 > & line, Point < F3 > & res) {
        res = line.a;
        if (line) res += line.ab * static_cast < F3 > ((point - line.a) * line.ab) / abs(line.ab);
    }

template < class F1, class F2, class F3 >
    void reflection(const Point < F1 > & point,
        const Line < F2 > & line, Point < F3 > & res) {
...
template < class F1, class F2, class F3 >
    void closest(const Point < F1 > & point,
        const Line < F2 > & seg, Point < F3 > & res) {
...
template < > struct EndpointChecker < 0 > { // no endpoint (ray)
    template < class F > bool operator()(const F & a,
        const F & b) const {
        return true;
    }
};
...
template < int LA, int LB, int RA, int RB, class F1, class F2, class F3 >
    bool intersectLines(const Line < F1 > & lhs,
        const Line < F2 > & rhs, Point < F3 > & res) {
        assert(lhs && rhs);
...
template < class F1, class F2, class F3 >
    bool intersectClosedSegments(const Line < F1 > & lhs,
        const Line < F2 > & rhs, Point < F3 > & res) {
        return intersectLines < 1, 1, 1, 1 > (lhs, rhs, res);
    }

template < class F1, class F2, class F3 >
    bool intersectSegments(const Line < F1 > & lhs,
        const Line < F2 > & rhs, Line < F3 > & res) {
        auto s = lhs.ab ^ rhs.ab;
        auto ls = (rhs.a - lhs.a) ^ rhs.ab;
        if (s == 0) {
...
template < class Iterator, class F >
    void sortByAngle(Iterator first, Iterator last,
        const Point < F > & origin) {
...
template < class F1, class F2 >
    int pointVsTriangle(const Point < F1 > & point,
        const Polygon < F2 > & triangle) {
        ...
        for (int i = 0; i < 3; ++i)
            if (signs[i] * signs[next(i, 3)] == -1) return 1;
        return 0;
    }

template < class F1, class F2 >
    int pointVsConvexPolygon(const Point < F1 > & point,
        const Polygon < F2 > & poly, int top) {
...
template < class F1, class F2 >
    int pointVsPolygon(const Point < F1 > & point,
        const Polygon < F2 > & poly) {
...
template < class F1, class F2 >
    pair < int, int > tangentsConvex(const Point < F1 > & point,
        const Polygon < F2 > & poly) {
        return {
            extremeVertex(poly, [ & point](const Point < F2 > & q) {
                return q - point;
            }),
            extremeVertex(poly, [ & point](const Point < F2 > & q) {
                return point - q;
...
template < class F1, class F2, class F3 >
    bool stabConvexPolygon(const Line < F1 > & line,
        const Polygon < F2 > & poly, Line < F3 > & res) {
        assert(line);
        int right = extremeVertex(poly, [ & line](const Point < F2 > & ) {
            return line.ab;
        });
        int left = extremeVertex(poly, [ & line](const Point < F2 > & ) {
            return -line.ab;
        });
...
template < class F1, class F2, class F = distF < F1, F2 >>
    F stabPolygonLength(const Line < F1 > & line,
        const Polygon < F2 > & poly) {
        assert(line);
        F tSum = F(0);
        int n = static_cast < int > (poly.size());
        auto vertexSgn = [ & line, & poly](int i) {
            return sgn(line.ab ^ (poly[i] - line.a));
        };
...
template < class F >
    bool point_in_convex_strict(const vector < Point < F >> & P,
        const Point < F > & Q)
    {
        size_t n = (int) P.size();
        ...
        // since ccw hence for Q to be inside , condition is > 0
    }
...
template < class F1, class F2 >
    bool polygon_in_convex_strict(vector < Point < F1 >> & outer,
        const vector < Point < F2 >> & inner)
    {
        // ensure A is CCW
...
template < class T >
    auto onSegment(const Point < T > & P1,
        const Point < T > & P2,
            const Point < T > & Q) -> decltype(2 == 3)
    {
...
template < class F >
    Point < F > intersection(const Point < F > & U,
        const Point < F > & V,
            const Point < F > & A,
                const Point < F > & B)
    {
        long double num = (U - A) ^ (B - A);
        long double den = (U - V) ^ (B - A);
        // if(den == 0) 
        long double t = num / den;
        return static_cast < Point < long double >> (U + t * (V - U));
    }

template < class F >
    Point < long double > intersection_safe(const Point < F > & U,
        const Point < F > & V,
            const Point < F > & A,
                const Point < F > & B)
    {
        long double eps = 1e-18 L;
        long double num = (U - A) ^ (B - A);
        long double den = (U - V) ^ (B - A);
        if (fabsl(den) < eps) {
            // parallel or nearly parallel.
            // return one endpoint as a fallback (shouldn't normally happen if logic is correct)
            return static_cast < Point < long double >> (U);
        }
        long double t = num / den;
        long double rx = U.x + t * (V.x - U.x);
        long double ry = U.y + t * (V.y - U.y);
        return Point < long double > (rx, ry);
    }



// (Only the main part is shown; keep your full template above.)
int32_t main()
{
    ios::sync_with_stdio(false);
    cin.tie(nullptr);

    const long double EPS = 1e-12 L;
    Point < long double > f;
    int n, w, h;
    int caseNo = 1;
    while (cin >> n >> w >> h >> f.x >> f.y) {
        vector < Point < long double >> poly = {{0, 0}, {(long double) w, 0}, {(long double) w, (long double) h}, {0, (long double) h}};
        int lines = n;
        while (lines--) {
            Point < long double > a, b;
            cin >> a.x >> a.y >> b.x >> b.y;
            long double c = (b - a) ^ (f - a);
            bool keepleft = c > EPS; // decide side using EPS

            vector < Point < long double >> next;
            int ps = (int) poly.size();
            if (ps == 0) continue;
            for (int i = 0; i < ps; ++i) {
                Point < long double > u = poly[i];
                Point < long double > v = poly[(i + 1) % ps];
                long double cu = (b - a) ^ (u - a);
                long double cv = (b - a) ^ (v - a);
                bool inU = keepleft ? (cu >= -EPS) : (cu <= EPS);
                bool inV = keepleft ? (cv >= -EPS) : (cv <= EPS);

                if (inU && inV) {
                    next.push_back(v);
                } else if (!inU && inV) {
                    next.push_back(intersection_safe(u, v, a, b));
                    next.push_back(v);
                } else if (inU && !inV) {
                    next.push_back(intersection_safe(u, v, a, b));
                } else {
            ...
            // remove near-duplicate consecutive vertices (optional but helpful)
            vector < Point < long double >> clean;
            for (auto & pt: next) {
                if (clean.empty()) clean.push_back(pt);
                else {
                    auto & last = clean.back();
                    if (fabsl(last.x - pt.x) > 1e-10 L || fabsl(last.y - pt.y) > 1e-10 L)
                        clean.push_back(pt);
                }
            }
            // also remove closing duplicate (first == last)
            if (clean.size() >= 2) {
                auto & first = clean.front();
                auto & last = clean.back();
                if (fabsl(first.x - last.x) < 1e-10 L && fabsl(first.y - last.y) < 1e-10 L)
                    clean.pop_back();
            }
        ...
        long double area = fabsl(ar) / 2.0 L;
        cout << "Case #" << caseNo++ << ": " << fixed << setprecision(3) << (double) area << "\n";
    }
    return 0;
}
...
// My minimal version code :

template<class F>
Point<long double> intersection_safe(const Point<F> &U, const Point<F> &V, const Point<F> &A, const Point<F> &B)
{
    long double eps = 1e-18L;
    long double num = (U - A) ^ (B - A);
    long double den = (U - V) ^ (B - A);
    // I dont know but tc with den == 0 wasn't there, means no test case with parallel segments were given
    // if(fabsl(den) < 1e-18L) return static_cast<Point<long double>>(U);
    long double t = num / den;
    long double rx = U.x + t*(V.x - U.x);
    long double ry = U.y + t*(V.y - U.y);
    return Point<long double>(rx, ry);
...
int32_t main()
{
    long double eps = 1e-18L; // eps also depends on compiler
    Point<long double> f;
    int n, w, h;
    int cases = 1;
    while(cin>>n>>w>>h>>f.x>>f.y) // UVa have this ICPC format input
    {
        // casting wont matter since intersection points will themselves be long double, also the area
        // long double wcp = static_cast<long double>(w);
        // long double hcp = static_cast<long double>(h);
        // vector<Point<long double>> poly = {{0, 0}, {wcp, 0}, {wcp, hcp}, {0, hcp}};
        vector<Point<long double>> poly = {{0, 0}, {w, 0}, {w, h}, {0, h}};
        while(n--)
        {
            Point<long double> a, b;
            cin>>a.x>>a.y>>b.x>>b.y;
            bool keepleft = ((b - a) ^ (f - a)) > 0;
            vector<Point<long double>> next;
            int ps = poly.size();
            for(int i = 0; i < ps; i++)
            {
                ...
                long double cu = (b - a) ^ (u - a);
                long double cv = (b - a) ^ (v - a);
                // Check if fountain and line is on same side, then keep points else not
                // Use eps for checking for floating integers checking or overflow condition
                // for checking >= 0, use as >= -eps, for <= 0, do as <= eps
                bool inU = keepleft ? (cu >= -eps) : (cu <= eps);
                bool inV = keepleft ? (cv >= -eps) : (cv <= eps);
                if(inU and inV) // segment fully inside on fountains side
                {
                    // next.emplace_back(intersection(u, v, a, b));
                    next.emplace_back(v);
                }
                else if(!inU and inV) // V only on fountains side, hence intersection point and V pushed
                {
                    next.emplace_back(intersection_safe(u, v, a, b));
                    next.emplace_back(v);
                }
                else if(inU and !inV) // U only on fountains side, hence intersection point pushed
                // U not pushed since U already have been pushed in previous iteration
                next.emplace_back(intersection_safe(u, v, a, b));
            }
            poly.swap(next); // swaps the buffer in O(1)
            // Swapping every time since next line should be checked with all lines inserted till now
        }
        long double ar = 0;
        n = poly.size();
        // n == 1 or n == 2 tc weren't present :(
        // if(n > 2)
        // {
            for(int i = 0, j = n - 1; i < n; j = i++)
            {
                Point<long double> p1 = poly[j % n];
                Point<long double> p2 = poly[i % n];
                ar += p1 ^ p2;
                // cout<<"i : "<<i<<" j : "<<j <<"\n";
            }
        // }
        // else
        // ar = 0.0L;
        cout<<"Case #"<<cases++<<": "<<fixed<<setprecision(3)<<ar/2<<"\n"; // Case #1: 1780.000 is UVa submission format
        // cout<<"Case #"<<cases++<<": "<<fixed<<setprecision(3)<<fasbl(ar)/2.0L<<"\n";
    }
}