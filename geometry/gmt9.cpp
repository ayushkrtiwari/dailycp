...
template<class F1, class F2>
int windingNumber(const Point<F1> &point, const Polygon<F2> &poly)
{
    size_t n = poly.size();
    int windingNumber = 0;
    for(int i = 0; i < n; i++)
    {
        if(point == poly[i]) return 0;
        int j = next(i, n);
        if(poly[i].y == point.y and poly[j].y == point.y)
        {
            if(min(poly[i].x, poly[j].x) <= point.x and point.x <= max(poly[i].x, poly[j].x)) return 0;
        }
        else
        {
            bool below = poly[i].y < point.y;
    ...
    return windingNumber == 0 ? inf : windingNumber;
}
...
// ---------- Minimal fountain Convex Hull, Area template (drop-in) ----------
static const long double FOUNTAIN_EPS = 1e-12L;

// intersect infinite line U->V with infinite line A->B
inline Point<long double> intersect_line(const Point<long double>& U,
                                        const Point<long double>& V,
                                        const Point<long double>& A,
                                        const Point<long double>& B)
{
    long double num = (U - A) ^ (B - A);   // (U-A) ^ (B-A)
    long double den = (U - V) ^ (B - A);   // (U-V) ^ (B-A)
    if (fabsl(den) < FOUNTAIN_EPS) {       // nearly parallel
        return U;                          // fallback
    }
    long double t = num / den;
    return Point<long double>(U.x + t*(V.x - U.x), U.y + t*(V.y - U.y));
}

// is P inside the half-plane of directed line A->B (keepLeft true => keep left side)
// poly vertices are long double; A,B,F may come from integer input converted to long double.
inline bool halfplane_contains(const Point<long double>& P,
                                const Point<long double>& A,
                                const Point<long double>& B,
                                bool keepLeft)
{
    long double cross = (B - A) ^ (P - A); // (B-A)^(P-A)
    if (keepLeft) return cross >= -FOUNTAIN_EPS;
    else return cross <=  FOUNTAIN_EPS;
}

// clip convex polygon 'poly' by half-plane (A->B, keeping side containing fountain)
inline vector<Point<long double>> clip_polygon_by_halfplane(const vector<Point<long double>>& poly,
                                                            const Point<long double>& A,
                                                            const Point<long double>& B,
                                                            bool keepLeft)
{
    vector<Point<long double>> out;
    int m = (int)poly.size();
    if (m == 0) return out;
    for (int i = 0; i < m; ++i) {
        Point<long double> U = poly[i];
        Point<long double> V = poly[(i+1)%m];
        bool inU = halfplane_contains(U, A, B, keepLeft);
        bool inV = halfplane_contains(V, A, B, keepLeft);
        if (inU && inV) {
            out.push_back(V);
        } else if (inU && !inV) {
            out.push_back(intersect_line(U, V, A, B));
        } else if (!inU && inV) {
            out.push_back(intersect_line(U, V, A, B));
            out.push_back(V);
        }
    }
    // remove near-duplicate consecutive points (avoid degeneracy)
    vector<Point<long double>> clean;
    for (auto &p : out) {
        if (clean.empty()) { clean.push_back(p); continue; }
        auto &q = clean.back();
        if (fabsl(p.x - q.x) > 1e-11L || fabsl(p.y - q.y) > 1e-11L) clean.push_back(p);
    }
    if (clean.size() >= 2) {
        auto &f = clean.front(), &b = clean.back();
        if (fabsl(f.x - b.x) < 1e-11L && fabsl(f.y - b.y) < 1e-11L) clean.pop_back();
    }
    return clean;
}

// shoelace area for polygon of Point<long double>
inline long double polygon_area(const vector<Point<long double>>& P)
{
    int n = (int)P.size();
    if (n < 3) return 0.0L;
    long double s = 0.0L;
    for (int i = 0, j = n-1; i < n; j = i++) s += (P[j] ^ P[i]);
    return fabsl(s) * 0.5L;
}

// main helper: given N, W, H, fountain coords (integers), and input stream (already at lines),
// returns area of the cell containing the fountain
inline long double fountain_cell_area(int N, int W, int H, long long fx, long long fy, istream &in = cin)
{
    Point<long double> F((long double)fx, (long double)fy);
    vector<Point<long double>> poly = {
        Point<long double>(0.0L, 0.0L),
        Point<long double>((long double)W, 0.0L),
        Point<long double>((long double)W, (long double)H),
        Point<long double>(0.0L, (long double)H)
    };

    for (int i = 0; i < N; ++i) {
        long long ax, ay, bx, by;
        in >> ax >> ay >> bx >> by;
        Point<long double> A((long double)ax, (long double)ay), B((long double)bx, (long double)by);
        long double cF = (B - A) ^ (F - A); // which side contains fountain
        bool keepLeft = cF > 0;            // fountain guaranteed not on a line
        poly = clip_polygon_by_halfplane(poly, A, B, keepLeft);
        if (poly.empty()) break;           // guard (shouldn't happen)
    }
...
int N, W, H;
long long fx, fy;
while (cin >> N >> W >> H >> fx >> fy) {
    long double area = fountain_cell_area(N, W, H, fx, fy);
...
int32_t main()
{
    int n, m;
    cin>>n>>m;
    Polygon<long double> poly(n);
    for(auto &e : poly) cin>>e.x>>e.y;
    if(area(poly) < 0) reverse(poly.begin(), poly.end());
    vector<Line<long double>> points(m);
    for(auto &e : points) 
    {
        long double x1, y1, x2, y2;
        cin>>x1>>y1>>x2>>y2;
        Point<long double> p1(x1, y1);
        Point<long double> p2(x2, y2);
        e = Line<long double>(p1, p2, true);
    }
    for(auto &e : points)
    {
        long double val = stabPolygonLength(e, poly);
        cout<<fixed<<setprecision(20)<<fabsl(val)<<"\n";
    }
}